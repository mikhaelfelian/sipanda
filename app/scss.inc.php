...
class scss_parser {
	 ...
	 * 
	 * @param mixed $out Reference to output value
	 * @return bool
	 */
	protected function parseValue(&$out)
	{
		// Initialize $out as empty array if not set
		if (!isset($out)) {
			$out = array();
		}
		...
		$out = $this->flattenList($expressions, $s);
	 ...
	 * 
	 * @param mixed $value Reference to output value
	 * @return bool
	 */
	protected function parseItem(&$value) 
	{
		// Initialize $value as empty array if not set
		if (!isset($value)) {
			$value = array();
		}
