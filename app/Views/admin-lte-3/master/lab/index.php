    ...
    <div class="col-12">
        <div class="card rounded-0">
            <div class="card-header">
                <div class="row">
                    <div class="col-md-6">
                        <a href="<?= base_url('master/lab/create') ?>" class="btn btn-sm btn-primary rounded-0">
                            <i class="fas fa-plus"></i> Tambah Data
                        </a>
                        <a href="<?= base_url('master/lab/export') ?>?<?= $_SERVER['QUERY_STRING'] ?>" class="btn btn-sm btn-success rounded-0">
                            <i class="fas fa-file-excel"></i> Export Excel
                        </a>
                        <a href="<?= base_url('master/lab/trash') ?>" class="btn btn-sm btn-danger rounded-0">
                            <i class="fas fa-trash"></i> Sampah (<?= $trashCount ?? 0 ?>)
                        </a>
                    </div>
                </div>
            </div>
            <div class="card-body table-responsive">
                <?= form_open(base_url('master/lab'), ['method' => 'get']) ?>
                        ...
                        <tr>
                            ...
                            <th>
                                <select name="kategori" class="form-control form-control-sm rounded-0">
                                    ...
                                    <?php foreach ($kategoriList as $id => $kategori): ?>
                                        <option value="<?= $id ?>" <?= $selectedKategori == $id ? 'selected' : '' ?>>
                                            <?= esc($kategori) ?>
                                        </option>
                                    <?php endforeach ?>
                                </select>
                            ...
                            <th>
                                <select name="merk" class="form-control form-control-sm rounded-0">
                                    ...
                                    <?php foreach ($merkList as $id => $merk): ?>
                                        <option value="<?= $id ?>" <?= $selectedMerk == $id ? 'selected' : '' ?>>
                                            <?= esc($merk) ?>
                                        </option>
                                    <?php endforeach ?>
                                </select>
                    ...
                    <tbody>
                        <?php foreach ($lab as $key => $row): ?>
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group">
                                        <a href="<?= base_url("master/lab/edit/$row->id") ?>"
                                            class="btn btn-warning btn-sm rounded-0">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="<?= base_url("master/lab/delete/$row->id") ?>"
